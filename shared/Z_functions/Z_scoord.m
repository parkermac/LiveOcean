...
if Vstretching == 1
    if theta_s ~= 0
        ...
        Cs_r = (1-theta_b)*cff1*sinh(theta_s*s_rho) ...
...
elseif Vstretching == 2
    ...
    if theta_s~=0 && theta_b~=0
        Csur = (1-cosh(theta_s.*s_rho))./(cosh(theta_s)-1);
        Cbot = ((sinh(theta_b.*(s_rho+1)))./(sinh(theta_b)))-1;
        u = ((s_rho+1).^alpha).*(1+(alpha/beta)*(1-((s_rho+1).^beta)));
        Cs_r = u.*Csur+(1-u).*Cbot;
        Csur_w = (1-cosh(theta_s.*s_w))./(cosh(theta_s)-1);
        Cbot_w = ((sinh(theta_b.*(s_w+1)))./(sinh(theta_b)))-1;
...
elseif Vstretching == 3
    ...
    gamma = 3;
    Csur = -(log(cosh(gamma.*abs(s_rho).^theta_s)))./log(cosh(gamma));
    Cbot = ((log(cosh(gamma.*(s_rho+1).^theta_b)))./log(cosh(gamma)))-1;
    mu = 0.5*(1-tanh(gamma*(s_rho+0.5)));
    Cs_r = mu.*Cbot+(1-mu).*Csur;
    Csur_w = -(log(cosh(gamma.*abs(s_w).^theta_s)))./log(cosh(gamma));
    Cbot_w = ((log(cosh(gamma.*(s_w+1).^theta_b)))./log(cosh(gamma)))-1;
...
elseif Vstretching == 4
    ...
    if theta_s>0
        Cs_r = (1-cosh(theta_s.*s_rho))./(cosh(theta_s)-1);
        Cs_w = (1-cosh(theta_s.*s_w))./(cosh(theta_s)-1);
    elseif theta_s<=0
        Cs_r = -(s_rho.^2);
        Cs_w = -(s_w.^2);
    end
