...
# ./driver_roms_new.sh -x lo1 -s new -r backfill -0 20140201 -1 20140203
#
# example call to do forecast:
# ./driver_roms_new.sh -x lo1 -s continuation -r forecast
#
...
do
  ...
  if [ $D = $D0 ] && [ $start_type = "new" ] ; then
    python ./make_dot_in_new.py $ex_name $start_type $run_type $DD
    cd $R_parent"/makefiles/"$ex_name"_tideramp"
  else
    python ./make_dot_in_new.py $ex_name continuation $run_type $DD
