...
qout_deep = np.nan * np.ones(NS)
qin_shallow = np.nan * np.ones(NS)
...
for LL in LList:
    ...
    qin[counter] = np.nansum(Q[:,1]/1e3)/ndg
    qout[counter] = np.nansum(Q[:,2]/1e3)/ndg
    
    qout_deep[counter] = np.nansum(Q[:,0]/1e3)/ndg
    qin_shallow[counter] = np.nansum(Q[:,3]/1e3)/ndg
    
    qsin[counter] = np.nansum(QS[:,1]/1e3)/ndg
    qsout[counter] = np.nansum(QS[:,2]/1e3)/ndg
...
# ax.plot(dist,-qout_deep,'--+r',linewidth=lw)
# ax.plot(dist,qin_shallow,'--+b',linewidth=lw)
ax.set_xlim(0,300)
ax.grid(True)
