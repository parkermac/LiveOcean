...
for sect_name in sect_list:
    ...
    # part without tidal pumping
    QQp_alt = VVp * AAp
    QQm_alt = VVm * AAm
    
    # form two-layer versions of Q and S
    Qin_alt = np.nansum(QQp_alt, axis=1)
    Qout_alt = np.nansum(QQm_alt, axis=1)
    ...
    this_qin_alt = tef_mean_df.loc[:,'Qin_alt'].to_numpy()/1e3
    this_qout_alt = tef_mean_df.loc[:,'Qout_alt'].to_numpy()/1e3
    ax.plot(this_yd, this_qin, '-*r', mfc='w', ms=ms, lw=lw)
    ax.plot(this_yd, this_qout, '-*b', mfc='w', ms=ms, lw=lw)
    ax.plot(this_yd, this_qin_alt, '-*', color='darkorange', mfc='w', ms=ms, lw=lw)
    ax.plot(this_yd, this_qout_alt, '-*', color='cornflowerblue', mfc='w', ms=ms, lw=lw)
    if do_Eulerian:
    ...
    ax.scatter(Time, QQp_alt/1e3, s=np.abs(qscl*QQp_alt/(1000*qlim)), c='darkorange', alpha=1)
    ax.scatter(Time, QQm_alt/1e3, s=np.abs(qscl*QQm_alt/(1000*qlim)), c='cornflowerblue', alpha=1)
