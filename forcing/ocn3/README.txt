...
Issue: since the old format (files in LiveOcean_data/hycom1) ended on 11/20/2018 we don't yet have a method for making archived backfill after that.  Also, the new format has a dofferent grid (twice the lat resolution).  The new version does make the same sort of h[].p files in Data, so perhaps we can use these.
...
Output: LiveOcean_output/[gtag]/[f_string]/ocn3/...
