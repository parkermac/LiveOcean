...
if exist(bryname,'file')
    ...
    % Create NO3 field
    if strcmp(NO3_method,'PL_Salt')
        % Calculate NO3 : PM Edits 5/27/2015 changed make_NO3_field
        % function call to be consistent with the function definition
...
if exist(bryname,'file')
    ...
    ncidg=netcdf.open(grdname, 'NC_NOWRITE');
    %     %% set the fill value to nothing so it will be zeros by default
    ...
    if strcmp(NO3_method,'PL_Salt')
        ...
        lat_rho = nc_varget(grdname,'lat_rho')';
...
if exist(rivname,'file')
    ...
    load('./bio_fun/River_TotalN_clm.mat')
    ...
    %     % from data
    vardn = datenum(1970,1,1) + vartime/86400;
    varyr = str2num(datestr(vardn,'yyyy'));
    varday = ceil(vardn - datenum(varyr,1,1));
    river_NO3(:,:,4) = repmat(CR_TotalN_clm(varday),1,size(river_salt,2)); % Columbia input 1
    river_NO3(:,:,5) = repmat(CR_TotalN_clm(varday),1,size(river_salt,2)); % Columbia input 2
    river_NO3(:,:,12) = repmat(FR_TotalN_clm(varday),1,size(river_salt,2)); % Fraser
    %
